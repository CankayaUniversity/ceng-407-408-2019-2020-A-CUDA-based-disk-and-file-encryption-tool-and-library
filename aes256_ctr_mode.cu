...
__device__ void AddRoundKey(uint8_t round, state_t* devState, const uint8_t* RoundKey)
{
  uint8_t i,j;
  ...
  for (i = 0; i < 4; ++i)
  {
    for (j = 0; j < 4; ++j)
    {
      ...
      (*devState)[i][j] ^= RoundKey[(round * Nb_d * 4) + (i * Nb_d) + j];
    	//(cipher)[i*4+j] = 'c';
    }
  }
}
...
__device__ void SubBytes(state_t* devState)
{
  uint8_t i, j;
  for (i = 0; i < 4; ++i)
  {
    for (j = 0; j < 4; ++j)
    {
      ...
      (*devState)[j][i] = sbox_d[(*devState)[j][i]];
    }
  }
}
...
__device__ void ShiftRows(state_t* devState)
{
  uint8_t temp;

  // Rotate first row 1 columns to left  
  temp           = (*devState)[0][1];
  (*devState)[0][1] = (*devState)[1][1];
  (*devState)[1][1] = (*devState)[2][1];
  (*devState)[2][1] = (*devState)[3][1];
  (*devState)[3][1] = temp;

  // Rotate second row 2 columns to left  
  temp           = (*devState)[0][2];
  (*devState)[0][2] = (*devState)[2][2];
  (*devState)[2][2] = temp;

  temp           = (*devState)[1][2];
  (*devState)[1][2] = (*devState)[3][2];
  (*devState)[3][2] = temp;

  // Rotate third row 3 columns to left
  temp           = (*devState)[0][3];
  (*devState)[0][3] = (*devState)[3][3];
  (*devState)[3][3] = (*devState)[2][3];
  (*devState)[2][3] = (*devState)[1][3];
  (*devState)[1][3] = temp;
}
...
__device__ void MixColumns(state_t* devState)
{
	uint8_t i;
	uint8_t Tmp, Tm, t;
	for (i = 0; i < 4; ++i)
	{
		t   = (*devState)[i][0];
		Tmp = (*devState)[i][0] ^ (*devState)[i][1] ^ (*devState)[i][2] ^ (*devState)[i][3] ;
		Tm  = (*devState)[i][0] ^ (*devState)[i][1] ;
		Tm = xtime(Tm); (*devState)[i][0] ^= Tm ^ Tmp ;
		Tm  = (*devState)[i][1] ^ (*devState)[i][2] ;
		Tm = xtime(Tm); (*devState)[i][1] ^= Tm ^ Tmp ;
		Tm  = (*devState)[i][2] ^ (*devState)[i][3] ;
		Tm = xtime(Tm); (*devState)[i][2] ^= Tm ^ Tmp ;
		Tm  = (*devState)[i][3] ^ t ;
		Tm = xtime(Tm); (*devState)[i][3] ^= Tm ^ Tmp ;
	}
}

// GPUCipher is the main function that encrypts the PlainText.
__global__ void GPUCipher(state_t* devState, const uint8_t* RoundKey, uint8_t* plain_text, int count)
{
	int id = threadIdx.x;
	uint8_t round = 0;
	unsigned i;
	int bi;
//	i = RoundKey[threadIdx.x];
//	int length=ceil((float)256/16);
	// Add the First round key to the state before starting the rounds.
	//for(i=(threadIdx.x*length);i<length;i++){
	if (id < count){
		AddRoundKey(0, devState, RoundKey);
		for (round = 1; ; ++round)
		{
			SubBytes(devState); // getSBoxValue !!!
			ShiftRows(devState);
			if (round == Nr_d) {
			  break;
			}
			MixColumns(devState);
			AddRoundKey(round, devState, RoundKey);
		}
		 //Add round key to last round
		AddRoundKey(Nr_d, devState, RoundKey);
	}
	for ( i = 0, bi = (16 - 1); bi >= 0; ++i,--bi)
	{
		/* inc will overflow */
		if (devState[bi][i] == (uint8_t *)255)
		{
			devState[bi][i] = 0;
			continue;
		}
	...
	bi = 0;
	plain_text[i] = plain_text[i] ^ (uint8_t*)devState[bi][i];
}
...
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, uint32_t length, uint8_t block_count , int flag, int count)
{
	...
	//			cudaDevAssist(cudaMemcpyToSymbol(Rcon_d, &Rcon, 11*sizeof(uint8_t), 0, cudaMemcpyHostToDevice), 922, true);
				...
				//cudaDevAssist(cudaMalloc((void**)&buffer_d, textLength*sizeof(uint8_t)), 453, true);
				// Time starting
				...
				GPUCipher<<<1,block_count>>>(devState,roundKey_d, plain_text_d,count);
			...
			plain_text_h[i] = (plain_text_h[i] ^ buffer2[bi]);
			buf = plain_text_h;
		}
	}
	else{ //CPU Encryption
...
int main(int argc, const char * argv[])
{
	...
	clock_t c_start, c_stop;
  	...
  	uint8_t key[] = "1110101ABC8671D11110101ABC8671D1";
  	//uint8_t in[2430];
    uint8_t iv[16] = { 0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0xff };
    FILE *infile, *outfile, *keyfile;
    int flag; //0 is CPU, 1 is GPU
    flag = 1;


//	printf("Enter -e- for encryption or enter -d- for decryption\n");
//	scanf("%c", &argv[0]); // Decide
	printf("Enter the path of input file");
	scanf("%s", argv[0]);
//	printf("Enter the path of key file");
//	scanf("%s", argv[2]);
...
//	uint8_t key[size_key];
	...
	//fread(&key, sizeof(char), size_key, keyfile);
	uint8_t block_count = (count % 16) + (count % 16 == 0 ? 0 : 1);
	//fseek(infile, 0, SEEK_END);
...
//	printf("Key File size: %d bytes\n", size_key);
	...
	if (flag == 1){
		...
		printf("\nInput File Length: %lu",strlen((char*)in));
		printf("Data read from file: %s\n", in);
		//printf("keyfile: %s\n", key);
		AES_init_ctx_iv(&ctx, key, iv);
		c_start = clock();
		AES_CTR_xcrypt_buffer(&ctx, in, strlen((char*)in),block_count,flag,count);
		...
		for(int i = 256, a=0; i<256+size;i+=256,a+=256){
			...
			AES_CTR_xcrypt_buffer(&ctx, Input, strlen((char*)Input),block_count,flag,count);
...
//    AES_init_ctx_iv(&ctx, key, iv);
//    for(int i = 256, a=0; i<size;i+=256,a+=256){
//
//    	memcpy(Input, in+(1*a),256);
//		AES_CTR_xcrypt_buffer(&ctx, Input, strlen((char*)in), block_count, flag, count);
//		printf("\nDEC: %s",(char*) Input);
//    }
